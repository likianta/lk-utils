...
class Signal:
    ...
    def __init__(self, *_) -> None:
        ...
        # )
        self.id = (stack.filename, stack.lineno, stack.line)  # type: ignore
        self._funcs = {}
    ...
    def bind(self, func: T.Func, name: str = '') -> T.FuncId:
        id = name or get_func_id(func)
        if id in self._funcs and config.duplicate_locals_scheme == 'ignore':
            return id
        self._funcs[id] = func
        return id
    ...
    def emit(self, *args, **kwargs) -> None:
        ...
        try:
            ...
            if config.circular_signal_error == 'prompt':
                print(
                    'signal is prevented because of circular emissions', ':p2vs'
                )
